...
<div class="content-wrapper">
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <form action="{{route('admin.researches.update',$research->id)}}" method="POST" class="mt-5" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="form-group">
                    <label for="name"><i class="fas fa-tasks"></i>&nbsp;Name</label>
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{$research->name}}">
                    @error('name')
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" name="member_id" aria-label="Default select example">
                        ...
                        <option value="{{$member->id}}" {{$member->id == $research->member_id ? 'selected' :''}}>{{$member->name}}</option>
                        @endforeach    
                    </select>
                </div>
                <div class="form-group">
                    <label for="image"><i class="fas fa-images"></i>&nbsp;Image</label><br>
                    <img src="{{ asset('storage/research/' . $research->image) }}" alt="Research Image" width="200" height="130"><br><br>
